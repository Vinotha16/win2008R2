...
if (Test-Path "Registry::HKEY_USERS\$sid" -PathType Container) {
    ...
    if ($test -eq 'True') {
        ...
        if ($value.PreventCodecDownload -eq 1) {
			...
			$count=($count+1)
			$regkeyval = (REG QUERY "HKEY_USERS\$sid\$path" | select-string 'PreventCodecDownload' 2> $null |  Measure-Object | %{$_.Count} )
			if ( $regkeyval -eq 1) {
				$query = (REG QUERY "HKEY_USERS\$sid\$path" | select-string 'PreventCodecDownload' 2> $null).ToString().Split('')[12].Trim()
				$fail = ([wmi]"Win32_SID.SID='$sid'" | Select-Object AccountName | where-object -filter {$_.AccountName} | Foreach {$_.AccountName})
			    Add-Content -Path C:\facts\prevcodefail.txt  -Value $fail-$query
			} else{
				$fail = ([wmi]"Win32_SID.SID='$sid'" | Select-Object AccountName | where-object -filter {$_.AccountName} | Foreach {$_.AccountName})
				Add-Content -Path C:\facts\prevcodefail.txt  -Value $fail-DELETE
			}
        }
    }
    else {
...
if ($count -ne '0') {
	if (Test-Path -Path "C:\facts\prevcode.txt") {	
	    ...
	    $outputfail = ( Get-Content -Path C:\facts\prevcodefail.txt | Sort-Object | Get-Unique ) -join ','
		Write-Output "$outputfail and SKIPPING $output"
		Remove-Item C:\facts\prevcode.txt 
		Remove-Item C:\facts\prevcodefail.txt
	} else {
	    $outputfail = ( Get-Content -Path C:\facts\prevcodefail.txt | Sort-Object | Get-Unique ) -join ','
