...
#   1.1.2 (L1) - Ensure 'Maximum password age' is set to '60 or fewer days, but not 0' (Scored)
	...
	if ($output1 -eq $unique) {
	    $output1 = $output1 | ForEach-Object {$_.replace('Unlimited', '0')}
		Write-Output "failed $output1"
	} elseif ([int]$output -ne [int]$unique){
	         $output = (net accounts | select-string 'Maximum password age ' | Foreach {"$(($_ -split '\s+'))"} | ForEach-Object {$_.replace('Maximum password age (days): ', '')})
