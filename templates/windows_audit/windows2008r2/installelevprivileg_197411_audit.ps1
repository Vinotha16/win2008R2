...
if (Test-Path "Registry::HKEY_USERS\$sid" -PathType Container) {
    ...
    if (Test-Path "Registry::HKEY_USERS\$sid\$path") {
		...
		$accname = [wmi]"Win32_SID.SID='$sid'" | Select-Object AccountName | where-object -filter {$_.AccountName} | Foreach {$_.AccountName} >> C:\facts\instel.txt        
}
}
if ($count -ne '0') {
	if (Test-Path -Path "C:\facts\instel.txt") {
		$output = ( Get-Content -Path C:\facts\instel.txt | Sort-Object | Get-Unique ) -join ','
		Write-Output "FAILED and SKIPPING $output"
		Remove-Item C:\facts\instel.txt
	} else {
		Write-Output "FAILED"
	}		
} else {
