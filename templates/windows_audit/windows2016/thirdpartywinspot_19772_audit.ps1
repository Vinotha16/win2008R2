...
if (Test-Path "Registry::HKEY_USERS\$sid" -PathType Container) {
    ...
    if (Test-Path "Registry::HKEY_USERS\$sid\$path") {
		...
		$accname = [wmi]"Win32_SID.SID='$sid'" | Select-Object AccountName | where-object -filter {$_.AccountName} | Foreach {$_.AccountName} >> C:\facts\thirpa.txt        
}
}

if ($count -ne '0') {
	if (Test-Path -Path "C:\facts\thirpa.txt") {
		$output = ( Get-Content -Path C:\facts\thirpa.txt | Sort-Object | Get-Unique ) -join ','
		Write-Output "FAILED and SKIPPING $output"
		Remove-Item C:\facts\thirpa.txt
	} else {
		Write-Output "FAILED"
	}		
} else {
