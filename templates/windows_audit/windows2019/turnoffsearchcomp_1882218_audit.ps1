...
#  	18.8.22.1.8 (L2) - Ensure 'Turn off Search Companion content file updates' is set to 'Enabled' (Scored)
	...
	if (( $path -eq 'True' ) -And ( $unique -eq '1' )) {
		foreach ( $unique1 in (REG QUERY "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\SearchCompanion" | select-string 'DisableContentFileUpdates').ToString().Split('')[12].Trim() ) {
